Thank you for downloading IWBT Studio Engine YoYoYo Edition v1.01!
...
-It is based off of the Yuutu engine and has most of its features and structure.
...
Features
-Works mostly the same as the Yuutu engine, with cleaner code and annoying bugs like dying through platforms fixed
...
-Several options for many things you might want to change for your game (such as enabling/disabling death music) in the "scrConfigVariables" script

Important notes
-To set what music you want a room to play, change the "scrGetMusic" script. You can also place an objPlayMusic object in the room and set the variable "BGM" in its creation code.
...
-Make sure to set the "global.debugMode" variable in the "scrConfigVariables" script to 0 before releasing your game, otherwise debug keys will still work.
-Make sure to set the "global.md5StrAdd" string in the "scrConfigVariables" script to something unique to your game to make saves harder to modify. This can be set to anything you want it to be, but it should be pretty hard to predict as it's kind of similar to a password.
...
-This version of the engine was made and tested in GM Studio v1.4.1567. Apparently they are changing how some collisions work in future versions, so if things break in newer versions of Studio I will try to update this engine to support it.